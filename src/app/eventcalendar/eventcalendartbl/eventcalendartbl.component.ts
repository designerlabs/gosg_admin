...
export class EventcalendartblComponent implements OnInit, OnDestroy {
  ...
  getEventData(count, size, lng) {
    this.loading = true;
    this.commonservice.readProtected('calendar', count, size, '', lng).subscribe(
  ...
  getFilterList(count, size, keyword, filterkeyword?) {
    ...
    if(keyword != "" && keyword != null && keyword.length != null && keyword.length >= 3) {
      this.kword = keyword;
      this.loading = true;
      this.commonservice.readProtected('calendar',count, size, keyword, this.languageId)
      .subscribe(data => {
