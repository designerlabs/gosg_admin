...
@Component({
  selector: 'app-usersunregister',
  templateUrl: './usersunregister.component.html',
  styleUrls: ['./usersunregister.component.css']
})
export class UsersunregisterComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.isActive = true;
    this.displayedColumns = ['no', 'username', 'email', 'identno', 'passport', 'action'];
  ...
  getUsersData(page, size) {
    ...
    this.commonservice.readProtected('usermanagement/public', page, size, '', this.languageId).subscribe(data => {
  ...
  getFilterList(page, size, keyword, filterVal) {
    ...
    if(filterVal == 2){  // by Email
      this.dataUrl = 'usermanagement/public';
      param = '&email='+keyword+'&page='+page+'&size='+size;
    }

    else if (filterVal == 3){ // by keywords
      this.dataUrl = 'usermanagement/public';
      param = '&id='+keyword+'&page='+page+'&size='+size;
    }
  ...
  deleteItem(id) {
    ...
    this.loading = true;
      ...
      this.commonservice.delete(id, 'usermanagement/public/').subscribe(
        data => {
            this.commonservice.errorHandling(data, (function(){
